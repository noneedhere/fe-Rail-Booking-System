...
import { Seat, isSeatLocked } from "@/app/types"
...
const DeleteSeat = ({ selectedSeat }: { selectedSeat: Seat }) => {
    ...
    const handleSubmit = async (e: FormEvent) => {
        try {
            e.preventDefault()
            const url = `${BASE_API_URL}/seat/${selectedSeat.id_seat}`
    ...
    const isLocked = isSeatLocked(selectedSeat).isLocked;
            ...
            <div className="relative group">
                ...
                {isLocked && (
                    <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 px-2 py-1 bg-gray-800 text-white text-xs rounded opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap z-50">
                        Cannot delete seat in active schedule
                    </div>
                )}
            </div>
            <Modal isShow={isShow} onClose={state => setIsShow(state)}>
                <form onSubmit={handleSubmit}>
                    <div className="sticky top-0 bg-white px-5 pt-5 pb-3 shadow">
                        <div className="w-full flex items-center">
                            <div className="flex flex-col">
                                <strong className="font-bold text-2xl text-black">Delete Seat</strong>
                    ...
                    <div className="p-5 text-black">
                        Are you sure you want to delete seat <strong>{selectedSeat.seat_num}</strong>
                        {selectedSeat.carriage && (
                            <> from carriage <strong>{selectedSeat.carriage.carriage_name}</strong></>
                        )}?
                    </div>
