...
const EditSeat = ({ selectedSeat, carriages }: { selectedSeat: Seat; carriages: Carriage[] }) => {
    ...
    const handleSubmit = async (e: FormEvent) => {
        ...
        try {
            ...
            if (data?.status) {
                toast(data?.message, { containerId: "toastSeat", type: "success", hideProgressBar: true })
                setIsShow(false)
                setTimeout(() => router.refresh(), 1000)
            } else {
                toast(data?.message || "Failed to update seat", { containerId: "toastSeat", type: "warning", hideProgressBar: true })
            }
        } catch (err) {
            console.error(err)
            toast("Something went wrong", { containerId: "toastSeat", type: "error", hideProgressBar: true })
        }
    }
