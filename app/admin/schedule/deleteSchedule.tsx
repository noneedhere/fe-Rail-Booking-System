...
import { Schedule, isScheduleLocked } from "@/app/types"
...
const DeleteSchedule = ({ selectedSchedule }: { selectedSchedule: Schedule }) => {
    ...
    const handleSubmit = async (e: FormEvent) => {
        try {
            e.preventDefault()
            const url = `${BASE_API_URL}/schedule/${selectedSchedule.id_schedule}`
    ...
    const isLocked = isScheduleLocked(selectedSchedule).isLocked;
            ...
            <Modal isShow={isShow} onClose={state => setIsShow(state)}>
                <form onSubmit={handleSubmit}>
                    <div className="sticky top-0 bg-white px-5 pt-5 pb-3 shadow">
                        <div className="w-full flex items-center">
                            <div className="flex flex-col">
                                <strong className="font-bold text-2xl text-black">Delete Schedule</strong>
                                <small className="text-slate-400 text-sm">This will delete the schedule and all associated seat bookings.</small>
                            </div>
                    ...
                    <div className="p-5 text-black">
                        <p>Are you sure you want to delete schedule <strong>{selectedSchedule.schedule_name}</strong>?</p>
                        <p className="text-sm text-gray-500 mt-2">
                            Route: {selectedSchedule.departure} → {selectedSchedule.destination}
                        </p>
                    </div>
